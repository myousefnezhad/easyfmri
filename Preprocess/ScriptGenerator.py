...
class ScriptGenerator:
        ...
        if setting.empty:
            ...
            #       for cnt in Counters[si]:
                ...
                for r in runs:
                    ...
                    ScriptOutputAddr = setParameters3(setting.Analysis, setting.mainDIR, s, r, t, c)
                    BOLDaddr = setParameters3(setting.BOLD,setting.mainDIR, s, r, t, c)
                    ...
                    # Conditions
                    ConditionFile =  setParameters3(setting.EventFolder, setting.mainDIR, s, r, t, c) + setting.CondPre + ".mat"
                    ...
                    if setting.Anat:
                        AnatAddr = setParameters3(setting.BET,setting.mainDIR, s,"", t, c)
                    ...
                    for cond in range(1, CondLen + 1):
                        ...
                        ConditionFile =  setParameters3(setting.EventFolder, setting.mainDIR, s, r, t, c) + setting.CondPre + "_" +  str(cond) + ".tab"
    ...
    def runbase(self,SettingFileName):
        import numpy as np
        import nibabel as nb
        import scipy.io as io
        from Preprocess.BIDS import load_BIDS
        ...
        if setting.empty:
            ...
            #       for cnt in Counters[si]:
                ...
                for r in runs:
                    ...
                    scriptFile.write("# Spin-history (currently obsolete)\nset fmri(sh_yn) 0\n\n")
                    scriptFile.write("# B0 fieldmap unwarping?\nset fmri(regunwarp_yn) 0\n\n")
                    scriptFile.write("# GDC Test\nset fmri(gdc) \"\"\n\n")  # Only for preprocessng
                    scriptFile.write("# EPI dwell time (ms)\nset fmri(dwell) 0.7\n\n")
                    scriptFile.write("# EPI TE (ms)\nset fmri(te) 35\n\n")
                    scriptFile.write("# % Signal loss threshold\nset fmri(signallossthresh) 10\n\n")
                    scriptFile.write("# Unwarp direction\nset fmri(unwarp_dir) y-\n\n")
                    ...
                    scriptFile.write("# Lowpass temporal filtering\nset fmri(templp_yn) 0\n\n")
                    scriptFile.write("# MELODIC ICA data exploration\nset fmri(melodic_yn) 0\n\n")
                    scriptFile.write("# Carry out main stats?\nset fmri(stats_yn) 0\n\n")   # 0 for preprocessing only
                    scriptFile.write("# Carry out prewhitening?\nset fmri(prewhiten_yn) 1\n\n")
                    scriptFile.write("# Add motion parameters to model\n# 0 : No\n# 1 : Yes\nset fmri(motionevs) 0\nset fmri(motionevsbeta) \"\"\nset fmri(scriptevsbeta) \"\"\n\n")    # 0 for preprocessing only
                    scriptFile.write("# Robust outlier detection in FLAME?\nset fmri(robust_yn) 0\n\n")
                    scriptFile.write("# Higher-level modelling\n# 3 : Fixed effects\n# 0 : Mixed Effects: Simple OLS\n# 2 : Mixed Effects: FLAME 1\n# 1 : Mixed Effects: FLAME 1+2\nset fmri(mixed_yn) 2\n\n")

                    scriptFile.write("# Higher-level permutations\nset fmri(randomisePermutations) 5000\n\n")
                    scriptFile.write("# Number of EVs\nset fmri(evs_orig) 1\nset fmri(evs_real) 2\nset fmri(evs_vox) 0\n\n")
                    ...
                    # This is for the preprocessing mode without any event or condition
                    scriptFile.write("# EV 1 title\nset fmri(evtitle1) \"\"\n\n")
                    scriptFile.write("# Basic waveform shape (EV 1)\n# 0 : Square\n# 1 : Sinusoid\n# 2 : Custom (1 entry per volume)\n# 3 : Custom (3 column format)\n# 4 : Interaction\n# 10 : Empty (all zeros)\nset fmri(shape1) 0\n\n")
                    scriptFile.write("# Convolution (EV 1)\n# 0 : None\n# 1 : Gaussian\n# 2 : Gamma\n# 3 : Double-Gamma HRF\n# 4 : Gamma basis functions\n# 5 : Sine basis functions\n# 6 : FIR basis functions\n# 8 : Alternate Double-Gamma\nset fmri(convolve1) 2\n\n")
                    scriptFile.write("# Convolve phase (EV 1)\nset fmri(convolve_phase1) 0\n\n")
                    scriptFile.write("# Apply temporal filtering (EV 1)\nset fmri(tempfilt_yn1) 1\n\n")
                    scriptFile.write("# Add temporal derivative (EV 1)\nset fmri(deriv_yn1) 1\n\n")
                    scriptFile.write("# Skip (EV 1)\nset fmri(skip1) 0\n\n")
                    scriptFile.write("# Off (EV 1)\nset fmri(off1) 30\n\n")
                    scriptFile.write("# On (EV 1)\nset fmri(on1) 30\n\n")
                    scriptFile.write("# Phase (EV 1)\nset fmri(phase1) 0\n\n")
                    scriptFile.write("# Stop (EV 1)\nset fmri(stop1) -1\n\n")
                    scriptFile.write("# Gamma sigma (EV 1)\nset fmri(gammasigma1) 3\n\n")
                    scriptFile.write("# Gamma delay (EV 1)\nset fmri(gammadelay1) 6\n\n")
                    scriptFile.write("# Orthogonalise EV 1 wrt EV 0\nset fmri(ortho1.0) 0\n\n")
                    scriptFile.write("# Orthogonalise EV 1 wrt EV 1\nset fmri(ortho1.1) 0\n\n")
                    scriptFile.write("# Contrast & F-tests mode\n# real : control real EVs\n# orig : control original EVs\nset fmri(con_mode_old) orig\nset fmri(con_mode) orig\n\n")
                    scriptFile.write("# Display images for contrast_real 1\nset fmri(conpic_real.1) 1\n\n")
                    scriptFile.write("# Title for contrast_real 1\nset fmri(conname_real.1) \"\"\n\n")
                    scriptFile.write("# Real contrast_real vector 1 element 1\nset fmri(con_real1.1) 1\n\n")
                    scriptFile.write("# Real contrast_real vector 1 element 2\nset fmri(con_real1.2) 0\n\n")
                    scriptFile.write("# Display images for contrast_orig 1\nset fmri(conpic_orig.1) 1\n\n")
                    scriptFile.write("# Title for contrast_orig 1\nset fmri(conname_orig.1) \"\"\n\n")
                    scriptFile.write("# Real contrast_orig vector 1 element 1\nset fmri(con_orig1.1) 1\n\n")

                    # The rest of common sections
                    scriptFile.write("# Contrast masking - use >0 instead of thresholding?\nset fmri(conmask_zerothresh_yn) 0\n\n")
                    scriptFile.write("")
