...
def ClassicNetworkAnalysisROI(X, L, Coord, ROI, Integration, Metric, AtlasImg, affine=np.eye(4), AtlasPath="/tmp/atlas.nii.gz", Threshold=0.95):
    listL = sorted(np.unique(L))
    print(f"List of labels {listL}")
    # Generate RegMap
    RegionMap = {}
    ...
    for nn, xxr in enumerate(XR):
        for regIndx, regID in enumerate(sorted(RegionMap.keys())):
            if regID in ROI:
                for j in range(regIndx + 1, AtlasRegNum):
                    Net[nn, regIndx, j] = Metric(xxr[:, regIndx], xxr[:, j])
                    Net[nn, j, regIndx] = Net[nn, regIndx, j]
                    print(f"Label: {nn}, Network: {regIndx}({regID}) vs {j} is compared.")
    NetMaxValue = np.max(Net[:])
    print(f"Network maximum value: {NetMaxValue}")
    NetThreshold = Threshold * NetMaxValue
    print(f"Network Threshold: {NetThreshold}")
