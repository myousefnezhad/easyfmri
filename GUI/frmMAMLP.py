...
class frmMAMLP(Ui_frmMAMLP):
    ...
    def show(self):
        ...
        # Activation
        ui.cbActivation.addItem("relu")
        ui.cbActivation.addItem("tanh")
        ui.cbActivation.addItem("logistic")
        ui.cbActivation.addItem("identity")

        # Solver
        ui.cbSolver.addItem("adam")
        ui.cbSolver.addItem("lbfgs")
        ui.cbSolver.addItem("sgd")

        # Learning Rate
        ui.cbLearningRate.addItem("constant")
        ui.cbLearningRate.addItem("invscaling")
        ui.cbLearningRate.addItem("adaptive")
        ...
        dialog.setWindowTitle("easy fMRI Multi-layer Perceptron Classifier - V" + getVersion() + "B" + getBuild())
    ...
    def btnConvert_click(self):
        ...
        # NesterovsMomentum
        NesterovsMomentum = ui.cbNesterovsMomentum.isChecked()

        # EarlyStopping
        EarlyStopping = ui.cbEarlyStopping.isChecked()

        # Activation
        Activation = ui.cbActivation.currentText()
        ...
        # Learning Rate init
        try:
            LearningRateInit = np.double(ui.txtLearningRate.text())
        except:
            msgBox.setText("Learning Rate (init) is wrong!")
        ...
        try:
            HiddenLayer = ui.txtHiddenLayers.text()
            if not len(HiddenLayer):
                msgBox.setText("Hidden Layer is wrong!")
                msgBox.setIcon(QMessageBox.Critical)
                msgBox.setStandardButtons(QMessageBox.Ok)
                msgBox.exec_()
                return False

            HiddenLayer = tuple(np.int32(HiddenLayer.replace("\'", " ").replace(",", " ").replace("[", "").replace("]","").split()))
            if not len(HiddenLayer):
                msgBox.setText("Hidden Layer is wrong!")
                msgBox.setIcon(QMessageBox.Critical)
                msgBox.setStandardButtons(QMessageBox.Ok)
        ...
        # Batch Size
        try:
            BatchSize = np.int32(ui.txtBatchSize.text())
            if BatchSize == 0:
                BatchSize = "auto"
        except:
        ...
        OutData = dict()
        OutData["ModelAnalysis"] = "Multi-layer Perceptron Classifier"
        ...
        for fold in range(FoldFrom, FoldTo + 1):
            ...
            try:
                clf = MLPClassifier(hidden_layer_sizes=HiddenLayer,activation=Activation,solver=Solver,\
                                    alpha=Alpha,batch_size=BatchSize,learning_rate=LearningRate,\
                                    learning_rate_init=LearningRateInit,power_t=Powert,max_iter=MaxIter,\
                                    shuffle=Shuffle,tol=Tol,verbose=Verbose,warm_start=WarmStart,momentum=Momentum,\
                                    nesterovs_momentum=NesterovsMomentum,early_stopping=EarlyStopping,\
                                    validation_fraction=ValidationFraction, beta_1=Beta1, beta_2=Beta2, epsilon=Epsilon)
        ...
        print("DONE.")
        msgBox.setText("Multi-layer Perceptron Classifier is done.")
